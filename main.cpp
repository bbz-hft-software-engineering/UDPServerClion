...
int main()
{
    ...
    if ((recv_len = recvfrom(udpSocket, message, MESSAGELENGTH, 0, (struct sockaddr *) &clientAddress, &slen)) == SOCKET_ERROR)
    {
        cout << "Daten empfangen fehlgeschlagen: " << WSAGetLastError() << endl;
        system("pause");
        return 1;
    }
    cout << "Daten empfangen von " << inet_ntoa(clientAddress.sin_addr) << " " << ntohs(clientAddress.sin_port) << ": " << message << endl;
